...
class AutorApiController{
    ...
    public function getAll($req, $res){
        ...
        if ($filtrarPor && !$this->model->existeColumna($filtrarPor)) {
            return $this->view->response(
                "No es posible filtrar por '$filtrarPor', campo inexistente",
                400
            );
        }
        if ($valor && !$this->model->existeValorEnColumna($filtrarPor, $valor)) {
            return $this->view->response(
        ...
        $autores = $this->model->getAutores($filtrarPor, $valor,$orderBy,$order);
